...
var (
	json = `{"name": "defu","age": 18,"email": "xxxx@qq.com"}`
	xml  = `<xml><name>defu</name><age>25</age><email>xxxx@qq.com</email></xml>`
	form = `name=defu&age=18&email=xxxx@qq.com&price=0.01`
...
func TestFormBinder_Bind(t *testing.T) {
	...
	if assert.NoError(t, err) {
		assert.Equal(t, "defu", user.Name)
		assert.Equal(t, 18, user.Age)
		assert.Equal(t, "xxx@qq.com", user.Email)
	}
}
